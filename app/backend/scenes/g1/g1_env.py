...
class G1Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, domain_rand_cfg, show_viewer=False, device="cuda"):
        ...
        # add plain
        self.plane = self.scene.add_entity(gs.morphs.URDF(
    ...
    def step(self, actions, x=0, y=0, angle=0):
        ...
        if torch.isnan(self.base_pos).any():
            nan_envs = torch.isnan(self.base_pos).any(
        ...
        self.reset_buf |= torch.abs(
            self.pelvis_pos[:, 2]) < self.env_cfg["termination_if_pelvis_z_less_than"]
    ...
    def randomize_mass(self):
        if (self.counter % int(self.domain_rand_cfg['push_interval_s']/self.dt) == 0):
            ...
            added_mass = float(torch.rand(
                1).item() * (added_mass_range[1] - added_mass_range[0]) + added_mass_range[0])
    ...
    def push_robots(self):
        ...
        push_env_ids = env_ids[self.episode_length_buf[env_ids] % int(
            self.domain_rand_cfg['push_interval_s']/self.dt) == 0]
